...
2. Use its compact `hostBrief` to make a plan. For material work, use the
   included Material Specialist report when present. Do not rely on raw MCP
   logs or request an API key for normal subscription-hosted use.
...
5. For visual work, call `render_blender_preview` and use the returned image
   for a concise host-driven critique before any targeted correction.
6. Keep deletion subject to the existing explicit safety gates.
